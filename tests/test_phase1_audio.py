...
try:
    ...
    print("Make sure Python dependencies are installed: pip install -r python/requirements.txt")
    sys.exit(1)
...
def test_audio_processing_latency():
    ...
    try:
        ...
        result = ces_client.process_file_data(file_data, filename=os.path.basename(wav_file))
        ...
        if processing_time < 100.0:
            ...
            print(f"❌ Audio processing latency exceeds Phase 1 requirement: {processing_time:.2f}ms > 100ms")
            success = False
...
def test_audio_quality_metrics():
    ...
    try:
        ...
        test_cases = [
            ...
            ("noise", (np.random.normal(0, 0.1, 4800) * 32767).astype(np.int16).tobytes()),  # White noise
        ]
...
def test_real_time_streaming_simulation():
    ...
    try:
        ...
        if real_time_success:
            ...
            print(f"❌ Real-time streaming requirements not met (max: {max_latency:.2f}ms > {chunk_duration_ms}ms)")
