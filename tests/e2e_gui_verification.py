...
action_methods = [
    "list_nodes",
    "get_node_info",
    "health_status",  # Node Management
    "submit_compute_task",
    "list_workers",
    "check_task_status",  # Compute
    "upload_file",
    "download_file",
    "list_files",  # Files
    "test_p2p_connection",
    "ping_all_nodes",
    "check_network_health",  # Communications
    "show_peers",
    "show_topology",
    "show_stats",  # Network
    "run_dcdn_demo",
...
print(
    f"  GUI action methods implemented: {len(methods_implemented)}/{len(action_methods)}"
)
...
if script_path.exists():
    ...
    print("  ❌ Management script not found")
...
print(
    f"✅ Critical Methods Present: {len([m for m in critical_methods if m in rpc_methods_found])}/{len(critical_methods)}"
)
...
print(
    f"✅ Infrastructure: {'Ready' if compose_path.exists() and script_path.exists() else 'Incomplete'}"
)
