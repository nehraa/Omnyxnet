...
class P2PStreamingNode:
    ...
    async def start_server(self):
        ...
        self.server = await asyncio.start_server(
            self.handle_peer_connection, "localhost", self.port
        )
    ...
    async def process_received_message(self, data, timestamp):
        ...
        try:
            ...
            print(
                f"📨 Node {self.node_id}: Received {message['type']} from node {message['node_id']}"
            )
            print(
                f"   Latency: {latency:.2f}ms, Compression: {message['original_size']}/{message['processed_size']} bytes"
            )
...
async def simulate_streaming_network():
    ...
    if media_files:
        ...
        streaming_tasks.append(
            stream_media_chunks(nodes[0], audio_file[1], "audio", chunk_size=8192)
        )
    ...
    if len(media_files) > 1:
        ...
        video_file = next(
            (f for f in media_files if "video" in f[0] or f[0].endswith(".mp4")),
            media_files[1],
        )
        # Use larger chunk size for big video files to test streaming performance
        chunk_size = (
            65536 if len(video_file[1]) > 5000000 else 16384
        )  # 64KB for >5MB files
        streaming_tasks.append(
            stream_media_chunks(nodes[1], video_file[1], "video", chunk_size=chunk_size)
        )
        print(
            f"🎬 Streaming {video_file[0]} ({len(video_file[1])/1024/1024:.1f}MB) with {chunk_size/1024:.0f}KB chunks"
        )
    ...
    for node in nodes:
        ...
        print(f"\n🟢 Node {stats['node_id']}:")
        print(
            f"   Sent: {stats['bytes_sent']:,} bytes ({stats['messages_sent']} messages)"
        )
        print(
            f"   Received: {stats['bytes_received']:,} bytes ({stats['messages_received']} messages)"
        )
    ...
    if total_stats["all_latencies"]:
        avg_network_latency = sum(total_stats["all_latencies"]) / len(
            total_stats["all_latencies"]
        )
        ...
        print(
            f"   Network latency - Avg: {avg_network_latency:.2f}ms, Min: {min_latency:.2f}ms, Max: {max_latency:.2f}ms"
        )

        if total_stats["all_compression_ratios"]:
            avg_compression = sum(total_stats["all_compression_ratios"]) / len(
                total_stats["all_compression_ratios"]
            )
        ...
        print(
            f"   Phase 1 latency target (<100ms): {'✅ PASS' if phase1_ok else '❌ FAIL'}"
        )
...
async def stream_media_chunks(node, media_data, media_type, chunk_size=8192):
    ...
    chunks_sent = 0
    for i in range(
        0, min(len(media_data), 100000), chunk_size
    ):  # Limit to 100KB for demo
