...
fn main() -> Result<()> {
    ...
    println!("  Latency target (<100ms): {}", if phase1_latency_ok { "✅ PASS" } else { "❌ FAIL" });
    println!("  Compression working: {}", if phase1_compression_ok { "✅ PASS" } else { "❌ FAIL" });
    
    // Output for parsing by test scripts
    if args.len() > 1 && args[1] == "--ces-test" {
        println!("CES_TEST_RESULT:compressed_size={},latency_ms={:.2},ratio={:.2}", 
