...
use crate::compute::types::{ComputeConfig, ComputeError, JobManifest, ChunkInfo, TaskStatus};
...
impl ComputeExecutor {
    ...
    pub fn split_data(
        ...
        debug!("Splitting {} bytes into chunks of ~{} bytes", data.len(), chunk_size);
    ...
    pub fn deserialize_chunks(&self, data: &[u8]) -> Result<Vec<Vec<u8>>, ComputeError> {
        ...
        for i in 0..num_chunks {
            if offset + 4 > data.len() {
                return Err(ComputeError::SerializationError(
                    format!("Truncated chunk header at chunk {}", i)
                ));
            }
            ...
            if offset + chunk_len > data.len() {
                return Err(ComputeError::SerializationError(
                    format!("Truncated chunk data at chunk {} (need {} bytes, have {})",
                            i, chunk_len, data.len() - offset)
                ));
            }
