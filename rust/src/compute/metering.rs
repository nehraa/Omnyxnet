...
impl ResourceUsage {
    ...
    pub fn limit_percentage(&self, limits: &ResourceLimits) -> ResourcePercentage {
        ResourcePercentage {
            ...
            time: (self.execution_time_ms as f64 / limits.max_execution_time_ms as f64 * 100.0) as u32,
        }
    }
}
...
impl Metering {
    ...
    pub fn add_memory(&self, bytes: u64) -> Result<(), MeteringError> {
        ...
        if new_usage > self.limits.max_memory_bytes {
            warn!("Memory limit exceeded: {} > {}", new_usage, self.limits.max_memory_bytes);
            self.interrupt();
    ...
    pub fn free_memory(&self, bytes: u64) {
        self.memory_bytes.fetch_sub(bytes.min(self.memory_bytes.load(Ordering::SeqCst)), Ordering::SeqCst);
    }
    ...
    pub fn add_cycles(&self, cycles: u64) -> Result<(), MeteringError> {
        ...
        if new_cycles > self.limits.max_cpu_cycles {
            warn!("CPU cycle limit exceeded: {} > {}", new_cycles, self.limits.max_cpu_cycles);
            self.interrupt();
    ...
    pub fn check_time(&self) -> Result<(), MeteringError> {
        ...
        if elapsed > self.limits.max_execution_time_ms {
            warn!("Time limit exceeded: {}ms > {}ms", elapsed, self.limits.max_execution_time_ms);
            self.interrupt();
