...
impl Cache {
    ...
    pub fn new(cache_dir: impl AsRef<Path>, max_entries: usize, max_size_bytes: usize) -> Result<Self> {
    ...
    pub async fn put_shard(&self, file_hash: &str, shard_index: usize, data: Vec<u8>) -> Result<()> {
        let key = format!("{}:{}", file_hash, shard_index);
    ...
    pub async fn put_manifest(&self, manifest: FileManifest) -> Result<()> {
        ...
        info!("Cached manifest: {} ({} shards)", manifest.file_hash, manifest.shard_count);
        Ok(())
    }
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match tokio::fs::read_to_string(&path).await {
                    Ok(json) => {
                        match serde_json::from_str::<FileManifest>(&json) {
                            Ok(manifest) => {
                                let mut cache = self.manifest_cache.write().await;
    ...
    pub async fn remove_manifest(&self, file_hash: &str) -> Result<bool> {
        ...
        if removed {
            ...
            let manifest_path = self.cache_dir.join("manifests").join(format!("{}.json", file_hash));
