...
fn demo_fec() {
    ...
    let packets: Vec<Packet> = (0..8).map(|i| Packet {
    ...
    if engine.can_recover(&group) {
        ...
        println!("  ✓ Successfully recovered {} lost packets!", recovered.len());
    } else {
...
fn demo_p2p() {
    ...
    for i in 1..=10 {
        ...
        println!("    Peer {}: ↓ {} MB, ↑ {} MB", i, downloaded / 1_000_000, uploaded / 1_000_000);
    }
