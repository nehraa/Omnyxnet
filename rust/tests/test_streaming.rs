...
/// - Session management
use pangea_ces::{
    AudioConfig, AudioStreamReceiver, AudioStreamSender, StreamConfig, StreamPacket, StreamType,
    StreamingSession,
};
...
fn test_audio_encoding() -> Result<()> {
    ...
    let pcm: Vec<i16> = (0..frame_size)
        .map(|i| {
            (AMPLITUDE_SCALE
                * (2.0 * std::f32::consts::PI * TEST_FREQUENCY_HZ * i as f32 / SAMPLE_RATE).sin())
                as i16
        })
        .collect();
