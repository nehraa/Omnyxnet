...
class NetworkMonitor:
    ...
    def collect_metrics(self):
        ...
        for client in self.clients:
            ...
            try:
                ...
                # Record metrics
                self.metrics["node_metrics"][node_key]["response_times"].append(
                    response_time
                )
        ...
        error_rate = (
            (total_attempts - success_count) / total_attempts
            if total_attempts > 0
            else 1
        )
    ...
    def start_monitoring(self, interval: float = 2.0, duration: int = 300):
        ...
        # Header
        print(
            f"{'Time':<8} {'Nodes':<6} {'Latency(ms)':<12} {'Success%':<9} {'Error%':<8} {'Status'}"
        )
        print("-" * 80)
    ...
    def analyze_results(self):
        ...
        for node_key, metrics in self.metrics["node_metrics"].items():
            ...
            if response_times:
                ...
                print(
                    f"    Success Rate: {len(response_times)/len(metrics['response_times'])*100:.1f}%"
                )
    ...
    def stress_test(self, max_load: int = 1000, step: int = 50):
        ...
        for load in range(step, max_load + 1, step):
            ...
            if success_rate < 0.5:
                print("    💀 Performance degraded - stopping stress test")
                break

        print("\n📊 Stress Test Summary:")
        print(
            f"    Max sustainable load: {max([r['actual_rps'] for r in results if r['success_rate'] > 0.9]):.1f} RPS"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor Pangea Net network performance"
    )
    parser.add_argument(
        "--nodes",
        nargs="+",
    ...
    parser.add_argument(
        "--monitor", type=int, default=60, help="Monitor duration in seconds"
    )
    parser.add_argument(
        "--interval", type=float, default=2.0, help="Monitoring interval in seconds"
    )
    parser.add_argument(
        "--stress", action="store_true", help="Run stress test instead of monitoring"
    )
