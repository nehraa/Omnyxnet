...
class LoadTester:
    ...
    def test_basic_connectivity(self) -> Dict:
        ...
        results = {
            ...
            'connection_success_rate': len(self.clients) / len(self.nodes) if self.nodes else 0,
            'node_data': []
        }
        ...
        for client in self.clients:
            ...
            for i in range(iterations):
                try:
                    ...
                    if (i + 1) % 20 == 0:
                        print(f"    Node {client.host}:{client.port} - {i+1}/{iterations} iterations")
    ...
    def test_concurrent_load(self, concurrent_requests: int = 50, duration_seconds: int = 30) -> Dict:
        ...
        for client in self.clients:
            ...
            node_result = {
                ...
                'avg_response_time_ms': statistics.mean(response_times) if response_times else -1,
    ...
    def test_stress_limits(self, max_connections: int = 1000) -> Dict:
        ...
        for conn_count in connection_counts:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=conn_count) as executor:
                ...
                futures = [executor.submit(test_connection, node) for node in node_assignments]
            ...
            result = {
                ...
                'avg_response_time_ms': (total_response_time / successful_connections * 1000) if successful_connections > 0 else -1,
                'test_duration_seconds': test_duration,
                'connections_per_second': successful_connections / test_duration if test_duration > 0 else 0
            }
            results['connection_results'].append(result)
            
            print(f"      {successful_connections}/{conn_count} successful ({result['success_rate']:.2%})")
            
            # Stop if success rate drops below 50%
            if result['success_rate'] < 0.5:
                print(f"      Stopping stress test - success rate too low")
                break
    ...
    def run_full_test_suite(self) -> Dict:
        ...
        concurrent = self.test_concurrent_load(concurrent_requests=20, duration_seconds=15)
        ...
        summary = {
            ...
            'avg_latency_ms': statistics.mean([n['avg_latency_ms'] for n in latency['node_latencies']]) if latency['node_latencies'] else -1,
            'max_concurrent_success_rate': max([n['success_rate'] for n in concurrent['node_results']]) if concurrent['node_results'] else 0,
            'max_stress_connections': max([r['successful_connections'] for r in stress['connection_results']]) if stress['connection_results'] else 0,
        }
        ...
        print(f"Max concurrent success rate: {summary['max_concurrent_success_rate']:.2%}")
...
def main():
    parser = argparse.ArgumentParser(description="Load test Pangea Net decentralized network")
