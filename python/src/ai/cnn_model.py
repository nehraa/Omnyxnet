...
class ModelManager:
    ...
    def train_step(self, sequences: List[List[float]], targets: List[float], 
                   optimizer: Optional[torch.optim.Optimizer] = None,
                   criterion: Optional[nn.Module] = None) -> float:
        """
        ...
        tensor = torch.tensor(padded_sequences, dtype=torch.float32).to(self.device)
        targets_tensor = torch.tensor(targets, dtype=torch.float32).to(self.device).unsqueeze(1)
