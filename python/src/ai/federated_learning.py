...
class ModelWeightManager:
    ...
    def deserialize_weights(
        ...
        logger.debug(f"Deserialized weights into model")
        return model
...
class LocalTrainer:
    ...
    def train_on_local_data(
        ...
        for epoch in range(epochs):
            ...
            avg_epoch_loss = epoch_loss / max(1, len(voice_data) // self.config.batch_size)
    ...
    def personalized_federated_average(
        ...
        for key in local_state.keys():
            personalized_state[key] = (
                alpha * local_state[key] +
                (1 - alpha) * global_state[key]
            )
...
class P2PFederatedLearning:
    ...
    def aggregate_with_peers(self) -> bool:
        ...
        for peer_id, weights in self.peer_models.items():
            ...
            model = ModelWeightManager.deserialize_weights(weights, model, compressed=True)
...
if __name__ == '__main__':
    ...
    logger.info("2. fl_coordinator.get_model_for_handshake() - share during Noise handshake")
