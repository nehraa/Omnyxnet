...
class ShardOptimizerNN(nn.Module):
    ...
    def __init__(self, input_size: int = 6, hidden_size: int = 32):
        ...
        self.confidence_head = nn.Sequential(
            nn.Linear(input_size, 16), nn.ReLU(), nn.Linear(16, 1), nn.Sigmoid()
        )
...
class ShardOptimizer:
    ...
    async def optimize_with_feedback(
        self, metrics: NetworkMetrics, file_size_bytes: int
    ) -> ShardConfig:
        """
        ...
        adjusted_config = ShardConfig(k=k, m=m, confidence=config.confidence)
    ...
    def train_from_history(self, epochs: int = 100, lr: float = 0.001):
        ...
        y_tensor = torch.tensor(list(zip(y_k, y_m)), dtype=torch.float32).to(
            self.device
        )
...
def _example_usage():
    ...
    async def run_example():
        ...
        config = await optimizer.optimize_with_feedback(
            metrics, file_size_bytes=10 * 1024 * 1024
        )
