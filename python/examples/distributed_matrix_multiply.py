...
def distributed_matrix_multiply():
    ...
    def execute(chunk: bytes) -> bytes:
        ...
        print(f"    🔢 Computed block ({i},{j}): {len(block_a)}x{len(block_a[0])} * {len(block_b)}x{len(block_b[0])}")
        return result
...
def compute_from_file(
    ...
    if a_cols != b_rows:
        raise ValueError(f"Matrix dimensions incompatible: {a_rows}x{a_cols} * {b_rows}x{b_cols}")
...
Examples:
  ...
  python distributed_matrix_multiply.py --file matrix.json --connect --host 192.168.1.100
    ...
    parser.add_argument('--size', '-s', type=int, default=10, help='Matrix size for generation')
    parser.add_argument('--generate', '-g', action='store_true', help='Generate random matrices')
    parser.add_argument('--connect', '-c', action='store_true', help='Connect to remote node')
    parser.add_argument('--host', type=str, default='localhost', help='Remote node host')
    parser.add_argument('--port', type=int, default=8080, help='Remote node port')
    parser.add_argument('--verify', '-v', action='store_true', help='Verify result with numpy')
