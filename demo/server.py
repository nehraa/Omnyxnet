    ...
    def connect_to_go_node(
        ...
        try:
            ...
            if client.connect():
                ...
                self.add_log(
                    f"🔗 Connected to Go node via Cap'n Proto ({target_host}:{target_port})",
                    "success",
                )
                return True
            else:
                self.add_log(
                    f"Could not connect to Go node at {target_host}:{target_port}",
                    "warning",
                )
                return False
...
async def startup_event():
    ...
    if GO_CLIENT_AVAILABLE:
        ...
        state.add_log(
            "⚠️  Cap'n Proto client not available - install pycapnp for real node connection",
            "warning",
        )
...
async def get_status():
    ...
    return {
        ...
        "nodes": {
            ...
            "online": sum(
                1 for n in state.data.get("nodes", []) if n["status"] == "online"
            ),
        },
...
async def connect_to_go(
    host: str = Query(default=None), port: int = Query(default=None)
):
    """
    ...
    if state.connected_to_go:
        return JSONResponse(
            ...
            content={
                ...
                "message": "Already connected to Go node",
            },
        )
    ...
    if state.connect_to_go_node(target_host, target_port):
        return JSONResponse(
            ...
            content={
                "status": "connected",
                "message": f"Connected to Go node at {target_host}:{target_port}",
            },
        )
    else:
        raise HTTPException(
...
async def get_logs():
    ...
    return {"logs": state.get_recent_logs(50), "total_logs": len(state.logs)}
    ...
    try:
        ...
        async with state.processing_lock:
            ...
            new_task = {
                ...
                "type": ["gradient_sync", "data_shard", "ai_inference"][
...
async def run_action(background_tasks: BackgroundTasks):
    ...
    state.add_log("📦 Starting execution...", "info")
...
async def reset_demo():
    ...
    """
    state.reset()
    return {"status": "success", "message": "Demo state reset to golden data"}
...
async def discover_nodes():
    ...
    for port in [8080, 8081, 8082]:
        try:
            ...
            if result == 0:
                discovered.append(
                    {"host": "localhost", "port": port, "available": True}
                )
        except Exception:
            pass
...
async def run_dcdn_demo(background_tasks: BackgroundTasks):
    ...
    async def demo_task():
        try:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(PROJECT_ROOT / "python" / "main.py"),
                    "dcdn",
                    "demo",
                ],
    ...
    return {"status": "started", "message": "DCDN demo started in background"}
...
async def run_dcdn_test(background_tasks: BackgroundTasks):
    ...
    return {"status": "started", "message": "DCDN tests started in background"}
